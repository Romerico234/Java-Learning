public class UnorderedArrayList extends ArrayListClass {
    ...
    public UnorderedArrayList merge(UnorderedArrayList second) {
        UnorderedArrayList result = new UnorderedArrayList(length + second.length);
        for (int i = 0; i < length; i++)
            result.insertEnd(list[i]);

        for (int i = 0; i < second.length; i++)
            result.insertEnd(second.list[i]);

        return result;
    }

    public void split(UnorderedArrayList first, UnorderedArrayList second, int key) {
        first.clearList();
        second.clearList();
        
        for (int i = 0; i < length; i++) {
            if (list[i] <= key) 
                first.insertEnd(list[i]);
            else 
                second.insertEnd(list[i]);
        }
    }
}